...
static const CGFloat InterCUQPanelWidth       = 280.0;
static const CGFloat InterCUQHeaderHeight     = 36.0;
static const CGFloat InterCUQRowHeight        = 44.0;
static const CGFloat InterCUQPadding          = 8.0;

@interface InterCameraUnlockQueuePanel ()
...
@property (nonatomic, strong) NSMutableArray<InterCameraUnlockEntry *> *queueEntries;
...
@implementation InterCameraUnlockQueuePanel
...
- (void)setupViews {
    ...
    NSView *headerBar = [[NSView alloc] initWithFrame:NSMakeRect(0,
                                                                  self.bounds.size.height - InterCUQHeaderHeight,
                                                                  self.bounds.size.width,
                                                                  InterCUQHeaderHeight)];
    ...
    // Header label
    self.headerLabel = [NSTextField labelWithString:@"🔒 Camera Requests"];
    self.headerLabel.frame = NSMakeRect(InterCUQPadding, 8,
                                        self.bounds.size.width - InterCUQPadding * 2 - 36,
                                        20);
    ...
    CGFloat actionRowY = self.bounds.size.height - InterCUQHeaderHeight - 28;
    CGFloat halfWidth = (self.bounds.size.width - InterCUQPadding * 3) / 2.0;

    self.approveAllButton = [[NSButton alloc] initWithFrame:NSMakeRect(InterCUQPadding, actionRowY, halfWidth, 22)];
    ...
    self.approveAllButton.toolTip = @"Approve all camera unlock requests";
    self.approveAllButton.hidden = YES;
    [self addSubview:self.approveAllButton];

    self.denyAllButton = [[NSButton alloc] initWithFrame:NSMakeRect(InterCUQPadding * 2 + halfWidth, actionRowY, halfWidth, 22)];
    ...
    self.denyAllButton.toolTip = @"Deny all camera unlock requests";
    self.denyAllButton.hidden = YES;
    [self addSubview:self.denyAllButton];

    // Empty state label
    self.emptyLabel = [NSTextField labelWithString:@"No camera unlock requests"];
    self.emptyLabel.frame = NSMakeRect(InterCUQPadding,
                                       self.bounds.size.height / 2 - 10,
                                       self.bounds.size.width - InterCUQPadding * 2, 20);
    ...
    CGFloat scrollHeight = self.bounds.size.height - InterCUQHeaderHeight - actionRowBarHeight - InterCUQPadding;
    ...
    self.tableView.rowHeight = InterCUQRowHeight;

    NSTableColumn *column = [[NSTableColumn alloc] initWithIdentifier:@"cuqEntry"];
...
#pragma unused(tableColumn)
    ...
    InterCameraUnlockEntry *entry = self.queueEntries[row];
    NSString *cellId = @"CUQueueEntryCell";

    NSTableCellView *cell = [tableView makeViewWithIdentifier:cellId owner:nil];
    if (!cell) {
        cell = [[NSTableCellView alloc] initWithFrame:NSMakeRect(0, 0, InterCUQPanelWidth, InterCUQRowHeight)];
        ...
        iconLabel.frame = NSMakeRect(InterCUQPadding, 11, 22, 22);
        ...
        // Approve button
        NSButton *approveBtn = [[NSButton alloc] initWithFrame:NSMakeRect(InterCUQPanelWidth - 148, 10, 64, 24)];
        approveBtn.tag = 302;
        ...
        // Deny button
        NSButton *denyBtn = [[NSButton alloc] initWithFrame:NSMakeRect(InterCUQPanelWidth - 78, 10, 64, 24)];
        denyBtn.tag = 303;
...
- (void)approveAction:(NSButton *)sender {
    ...
    if (identity && [self.delegate respondsToSelector:@selector(cameraUnlockQueuePanel:didApproveParticipant:)]) {
        [self.delegate cameraUnlockQueuePanel:self didApproveParticipant:identity];
    }
}

- (void)denyAction:(NSButton *)sender {
    NSString *identity = sender.cell.representedObject;
    if (identity && [self.delegate respondsToSelector:@selector(cameraUnlockQueuePanel:didDenyParticipant:)]) {
        [self.delegate cameraUnlockQueuePanel:self didDenyParticipant:identity];
    }
}
...
#pragma unused(sender)
    ...
    if ([self.delegate respondsToSelector:@selector(cameraUnlockQueuePanelDidApproveAll:)]) {
        [self.delegate cameraUnlockQueuePanelDidApproveAll:self];
    }
}
...
#pragma unused(sender)
    if (self.queueEntries.count == 0) return;
    if ([self.delegate respondsToSelector:@selector(cameraUnlockQueuePanelDidDenyAll:)]) {
        [self.delegate cameraUnlockQueuePanelDidDenyAll:self];
    }
}
